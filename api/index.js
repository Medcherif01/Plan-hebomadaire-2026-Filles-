...
if (process.env.GEMINI_API_KEY) {
    ...
    // ===== CORRECTION FINALE : Utilisation du modèle compatible 'gemini-pro' =====
...
// ===== ROUTE POUR LE PLAN HEBDOMADAIRE (INCHANGÉE) =====
...
// ===== ROUTE POUR LE PLAN DE LEÇON IA (CORRIGÉE) =====
app.post('/api/generate-ai-lesson-plan', async (req, res) => {
    try {
        ...
        if (weekDates?.start && jour) {
            ...
            if (specificDate) {
                 calculatedDate = formatDateFrenchNode(specificDate).split(' ').slice(1).join(' ');
            }
        }
